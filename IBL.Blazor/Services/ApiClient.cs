...
public class ApiClient
{
    ...
    {
        using var response = await _http.GetAsync($"Container/GetTelemetriesOutOfRange?containerId={containerId}");
        var text = await ReadTextResponse(response);
        if (string.IsNullOrWhiteSpace(text) || !text.TrimStart().StartsWith("["))
        {
            return new List<string>();
        }

        return System.Text.Json.JsonSerializer.Deserialize<List<string>>(text, new System.Text.Json.JsonSerializerOptions
        {
